...
router.post('/', function (req, res) {
    ...
    function SearchID() {
        ...
        this.select=function(callback,id){
            var sql = 'SELECT distinct * FROM Seller where user_sid = ' + id;
            var option = {};
    ...
    if( sid.length !== 10 || sid[0] != 1 || sid[1] != 1 || sid[2] != 5 || sid[3] != 5){
    ...
    }  else if(category==''||college==''||object==''||price_min==''||price_max=='') {
        ...
        SeID.select(function(rdata){
            if(rdata[0].object=="Nothing"){
                var  addSql = 'INSERT INTO ready_match (request_id, user_sid, category, college, object, price_min, price_max, if_matched) VALUES(0,?,"S",?,?,?,?,0)';
