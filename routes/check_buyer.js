...
router.post('/', function (req, res) {
  ...
  function SearchID() {
    ...
    this.select=function(callback,id){
      var sql = 'SELECT distinct * FROM post where user_sid = ' + id + ' AND category = "Buyer"';
      var option = {};
  ...
  function CheckID(){
    this.select=function(callback,id,pid){
      var sql = 'SELECT * FROM match_result where user_id2 = ' + id  +' AND result != 0 AND pid2 = ' + pid;
      var option = {};  
      pool.query(sql,function(err,result){
          if(err){console.log(err);}
          option[0]={'user_id1':"00000",'user_id2':null,'result':null,'pid2':0};
          if(result){
              for(var i = 0; i < result.length; i++)
              {   
      ...
      pool.query(sql,[postId],function(err,result){
        ...
        if(result){
            for(var i = 0; i < result.length; i++)
            {
                ...
                'user_id1':result[i].user_id1,'user_id2':result[i].user_id2, 'pid2':result[i].pid2,
  ...
  function ReCheckID(){
    this.select=function(callback,id,pid){
        var postId = pid;
        var sql = 'SELECT * FROM match_result where user_id2 = ' + id + ' AND result = 0 AND pid2 =?';
        var option = {};  
        pool.query(sql,[postId],function(err,result){
            if(err){console.log(err);}
            option[0]={'user_id2':"00000",'pid2':0};
          ...
          // for(let i=0; i<postList_len; i++) {
            ChID.select(function (rdataC){
              ...
              // there is already a matching result
                ...
                var otherID;
                if((datasC[0].user_id2==userID)&&(datasC[0].pid2==AllPid[i])) {
